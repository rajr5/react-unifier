...
const layoutStyles = require("./Layout.module.css");

interface Props {
  Item: (arg0: {idx: number}) => React.ReactNode;
  layout: {
    top: number;
    left: number;
    width: number;
...
export default class Collection extends React.PureComponent<Props, void> {
  ...
  render() {
    ...
    // layer.
    const items: any = layout.reduce((acc, position, idx) => {
      if (
