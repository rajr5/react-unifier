...
const styles = require("./Contents.module.css");
...
type MainDir = ("up" | "right" | "down" | "left") | null | undefined;
...
interface State {
  ...
  mainDir: MainDir | null | undefined;
  flyoutRef: HTMLElement | null | undefined;
}
...
export function getMainDir(
  ...
  if (idealDirection && spaces[DIR_INDEX_MAP[idealDirection]] > 0) {
    ...
    mainDir = (SPACES_INDEX_MAP as any)[spaces.indexOf(max)];
  }
  return mainDir;
}
 ...
 */
export function calcEdgeShifts(subDir: SubDir, triggerRect: ClientRect, windowSize: Window) {
...
export default class Contents extends React.Component<Props, State> {
  ...
  static getDerivedStateFromProps(
    ...
    // Now that we have the main direction, chose from 3 caret placements for that direction
    const subDir = getSubDir(flyoutSize, mainDir, triggerRect, windowSize) as SubDir;
  ...
  setFlyoutRef = (flyoutRef: HTMLElement | null | undefined) => {
    ...
    return (
      <div className={styles.container} style={{stroke, visibility, ...flyoutOffset}}>
        ...
        >
          ...
          {caret && (
            <div className={classnames(colors[bgColor], styles.caret)} style={{...caretOffset}}>
