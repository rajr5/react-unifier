...
test("IconButton renders with icon", () => {
  const component = renderer.create(<IconButton accessibilityLabel="Pinterest" icon="pin" />);
...
test("IconButton renders with svg", () => {
  const component = renderer.create(
    <IconButton accessibilityLabel="Pinterest" dangerouslySetSvgPath={{__path: "M13.00,20.00"}} />
  );
  const tree = component.toJSON();
  expect(tree).toMatchSnapshot();
});
